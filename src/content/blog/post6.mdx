...
1. **Variables**: These are like containers for storing data. You can declare a variable using <InlineCode code={`let`} lang="javascript" />, <InlineCode code={`const`} lang="javascript" />, or <InlineCode code={`var`} lang="javascript" />.
...
4. **Conditionals**: <InlineCode code={`if`} lang="javascript" />, <InlineCode code={`else`} lang="javascript" />, and <InlineCode code={`switch`} lang="javascript" /> let you handle different scenarios based on conditions.
5. **Loops**: <InlineCode code={`for`} lang="javascript" />, <InlineCode code={`while`} lang="javascript" />, and <InlineCode code={`forEach`} lang="javascript" /> are your best friends for repeating actions.
...
- **Line 1** uses <InlineCode code={`prompt`} lang="javascript" /> to get input from the user.
- **Line 2** uses template literals (the backticks) to insert <InlineCode code={`userName`} lang="javascript" /> into the string.
- **Lines 3–7** define a function to add two numbers, then return the result.
- **Line 10** calls <InlineCode code={`addNumbers()`} lang="javascript" /> and logs the result to the console.
